...
const LoanPayoffWidget = memo(function LoanPayoffWidget({
  budgetData,
  monthsPassed,
}: LoanPayoffWidgetProps) {
  const loanItems = useMemo(() => 
    budgetData.filter((item) => item.category === "Loan"), 
    [budgetData]
  );
  ...
  return (
    <div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden dark-mode-transition">
      ...
      <ul className="divide-y divide-gray-200 dark:divide-gray-700">
        {loanItems.map((item) => (
          <LoanItem key={item.id} item={item} monthsPassed={monthsPassed} />
        ))}
      </ul>
    </div>
  );
...
const LoanItem = memo(function LoanItem({ 
  item, 
  monthsPassed 
}: { 
  item: BudgetItem; 
  ...
  const { difference, totalLoanAmount, totalPaid } = useMemo(() => {
    ...
    return { difference, totalLoanAmount, totalPaid };
  }, [item, monthsPassed]);
...
export default LoanPayoffWidget;
